...
const props = defineProps({
  ...
  sectionTitle: { type: String, default: 'Pasūtījumi' },
  emptyText: { type: String, default: 'Pasūtījumu nav.' },
})
...
async function loadOrders() {
  ...
  try {
    const response = await authFetch(props.endpoint)
...
.sectionHeader { align-items: center; margin: 22px 0 12px; padding: 0 2px; }
...
@media (max-width: 820px) { .topbar { align-items: flex-start; flex-direction: column; } .orderTop, .itemLine { grid-template-columns: 1fr; display: grid; } .cost { text-align: left; } }
</style>
